...
This guide defines logging conventions for Polyphony so Codex can diagnose failures quickly.
...
- `issue_id`: tracker-native stable identifier (for GitHub, issue node ID when available).
- `issue_identifier`: human issue key (for GitHub, `#123`).
...
- Is this event tied to a tracker issue? Include `issue_id` and `issue_identifier`.
