defmodule Mix.Tasks.Workspace.BeforeRemoveTest do
  ...
  test "uses current branch for lookup when branch option is omitted" do
    with_fake_gh_and_git(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo your-org/polyphony --head feature/workpad --state open --json number --jq .[].number"

        assert log =~ "pr close 101 --repo your-org/polyphony"
        assert log =~ "pr close 102 --repo your-org/polyphony"
      end
    )
  end
  ...
  test "closes open pull requests for the branch and tolerates close failures" do
    with_fake_gh(fn log_path ->
      ...
      assert log =~ "pr list --repo your-org/polyphony --head feature/workpad --state open --json number --jq .[].number"
      assert log =~ "pr close 101 --repo your-org/polyphony"
      assert log =~ "pr close 102 --repo your-org/polyphony"
  ...
  test "formats close failures without command stderr output" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        log = File.read!(log_path)
        assert log =~ "pr list --repo your-org/polyphony --head feature/no-output --state open --json number --jq .[].number"
        assert log =~ "pr close 102 --repo your-org/polyphony"
      end
    )
  end
  ...
  test "no-ops when PR list fails for current branch" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo your-org/polyphony --head feature/list-fails --state open --json number --jq .[].number"
